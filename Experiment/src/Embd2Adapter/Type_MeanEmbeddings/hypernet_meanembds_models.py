...
class HyperNetController_MeanEmbds(nn.Module):
    ...
    def forward(self, embedding, layer_id):
        if embedding.ndim == 3:
        ...
        if embedding.ndim != 1 or embedding.shape[0] != self.embd_dim:
            raise ValueError(
                f"Expected embedding shape ({self.embd_dim},), "
                f"(n, {self.embd_dim}), or (1, n, {self.embd_dim})."
            )
...
class HyperNetWrapper_MeanEmbds(nn.Module):
    ...
    def mean_embeddings(self, embeddings):
        ...
        if embeddings.ndim == 3:
            if embeddings.shape[0] != 1:
                raise ValueError("HyperNet currently supports batch_size=1 only.")
            return embeddings[0].mean(dim=0)
        raise ValueError(
            "embeddings must have shape (dim,), (n, dim), or (1, n, dim)."
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        embedding = self.mean_embeddings(embeddings=embedding)
