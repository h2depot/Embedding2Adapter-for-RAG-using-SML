...
from .hypernet_meanembds_trainer import HyperNetMeanEmbdsTrainer


class HyperNetMeanEmbdsPipeline:
    def __init__(self, dataset_constructor, embd_model):
        self.embd_model = embd_model
        self.trainer = HyperNetMeanEmbdsTrainer(dataset_constructor, embd_model)
        print("Hypernet with MeanEmbds Loaded.")
    ...
    def load_trained_hypernet(self, checkpoint_path=None):
        if checkpoint_path is None:
            checkpoint_path = (
                Path(self.trainer.info["training"]["output_dir_mean_embds"])
    ...
    def generate(self, context, query, context_embeddings, query_embedding = None):
